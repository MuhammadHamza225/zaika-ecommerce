...
const Store = () => {
  ...
  return (
    <div className="py-20 px-0 text-center">
      ...
      {/* Grid layout: 4 items per row, center second row manually if needed */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[2px] px-0 mx-0">
        {products.map((product, idx) => (
          <div key={idx} className="relative w-full p-0.5 group">
            {product.tag && (
            ...
            <img
              src={product.image}
              alt={product.title}
              className="w-[22rem] h-80 object-cover hover:cursor-pointer"
            />
      ...
      <button className='mt-5 px-4 py-2 border-2 border-black text-[15px] font-semibold  hover:text-white transition cursor-pointer hover:bg-[#ff695c]'>SHOP NOW</button>
    </div>
  );
};
